...
export function getDogs() {
    return async function (dispatch) {
        var json = await axios.get('http://localhost:3001/routesDog/dogs')
        return dispatch({
            type: 'GET_DOGS',
            payload: json.data
        })
...
export function getDogsByName(name) {
    return async function (dispatch) {
        const { data } = await axios.get(`http://localhost:3001/routesDog/dogs?name=${name}`);
        return dispatch({
...
export function getDetails(id) {
    return async function (dispatch) {
        try {
            var json = await axios.get(`http://localhost:3001/routesDog/dogs/${id}`)
            return dispatch({
                type: 'GET_DETAILS',
                payload: json.data
            })
