        ...
        try {
            ...
            if (name) { /* Si entra un query */
                ...
                dogName.length ?
                    ...
                    res.status(404).send("No encuentro el perro con el nombre que buscas")
...
dogs.post('/dogs', async (req, res) => {
    ...
    if (name && height_min && height_max && weight_min && weight_max && temperament && image) {
        // toma esos datos para el nuevo perro 
...
dogs.get('/dogs/:idRaza', async (req, res) => {
    ...
    try {
        ...
        if (!idRaza) {
            res.status(404).json("No se pudo encontrar el nombre en DBase")
        } else {
